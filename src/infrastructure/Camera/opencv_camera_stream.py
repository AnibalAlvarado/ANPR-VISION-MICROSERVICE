...
class OpenCVCameraStream(ICameraStream):
    ...
    Siempre expone el último frame disponible (sin acumular retraso).
    """
    ...
    def connect(self) -> None:
        ...
        if self.url.startswith("rtsp://") and self.cap is not None:
    ...
    def _update_frames(self):
        ...
        while self._running:
            ...
            with self._frame_lock:
                self._latest_frame = Frame(data=frame, timestamp=time.time(), source=self.url)
    ...
    def _try_reconnect(self) -> bool:
        ...
        for attempt in range(1, self.reconnect_attempts + 1):
            ...
            if self.url.startswith("rtsp://") and cap is not None:
